...
func render_worker():
	...
	while is_worker_running:
		if render_queue.size() > 0:
			...
			var new_texture:Texture2D = get_page_image(render_page_index,350)
...
func go_to_next_page(delta) -> void:
	...
	var new_texture: Texture2D = get_page_image(current_page,25)
	...
	set_scale(Vector2(slow_dpi/fast_dpi, slow_dpi/fast_dpi))
	print(scale)
	texture = new_texture
...
func _update_stroke_page():
	$StrokeRenderer.go_to_page(current_page,texture.get_width()/2,texture.get_height()/2)
	$StrokeRenderer.scale = Vector2(2,2)
